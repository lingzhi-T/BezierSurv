'''
In this version, the data is stored directly in GPU memory, 
which speeds up training. Since CPU usage becomes lower in this setup.
'''
#----> pytorch imports
import torch
...
from datasets.dataset_survival import SurvivalDatasetFactory_0731
...
def main(args):
    ...
    for i in folds:
        # if i == 4:
            ...
            results, (val_cindex, val_cindex_ipcw, val_BS, val_IBS, val_iauc, total_loss),result_save_fold = _train_val_0731(datasets,
                                                                                                                              i, args)
            ...
            end_time = time.time()
            wall_time_hours = (end_time - start_time) / 3600.0

            # 实际并行使用的 GPU 数，单卡就 1，多卡就 torch.cuda.device_count()
            num_gpus = torch.cuda.device_count() if torch.cuda.is_available() else 1
            total_gpu_hours = wall_time_hours * num_gpus

            print(f"[Compute] wall-clock time : {wall_time_hours:.2f} hours")
            print(f"[Compute] total GPU-hours : {total_gpu_hours:.2f} (using {num_gpus} GPU(s))")
...
if __name__ == "__main__":
    ...
    args.dataset_factory = SurvivalDatasetFactory_0731(
        study=args.study,
