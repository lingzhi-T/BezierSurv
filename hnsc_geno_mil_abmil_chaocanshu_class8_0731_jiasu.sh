...
# 设置网格搜索的超参数列表
MIL_MODEL_TYPES=("TransMIL")   # 可选的 MIL 模型类型
GENO_MODEL_TYPES=("mlp")  # 可选的基因类型
        ...
        for N_CTRL_POINTS in "${CTRL_POINTS[@]}"; do  # 遍历不同的控制点数
          for N_CLASSES in "${N_CLASSES[@]}"; do    # 遍历不同的类别数
            ...
            CUDA_VISIBLE_DEVICES=9 python main_bezier_20250729_jiasu.py \
              ...
              --modality BezierSurv \
              --data_root_dir /media/raid/****/svs_file/bingli_20250502/hnsc_processed/feat-l1-unvi-B-new/pt_files \
                --label_file datasets_csv/metadata/tcga_hnsc.csv \
                --omics_dir datasets_csv/raw_rna_data/combine/hnsc \
              --results_dir results_hnsc_ours_spl_on_censored_only_n_classes_${N_CLASSES}_lr_${LR}_mil_${MIL_MODEL}_geno_${GENO_MODEL}_seed_${SEED}_ctrl_points_${N_CTRL_POINTS}_jiasu_val \
