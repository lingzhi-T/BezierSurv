...
class SurvivalDatasetFactory:
    ...
    def _discretize_survival_months(self, eps, uncensored_df):
        ...
        if self.discretize_strategy == 'quantile':
            ...
            self.bins = q_bins
        elif self.discretize_strategy == 'hybrid':
            assert self.n_bins >= 2, "Hybrid strategy requires at least 2 bins."

            # Step 1: 获取固定区间左边界
            max_time = uncensored_df[self.label_col].max()
            full_max = self.label_data[self.label_col].max()
            full_min = self.label_data[self.label_col].min()

            # 1) 前 n_bins-1 段：等距
            fixed_bins = np.linspace(full_min - eps, max_time, self.n_bins)
            ...
            last_edge = full_max + eps
            q_bins[-1] = last_edge

            # 拼接 bins
            time_bins = np.concatenate([fixed_bins[:-1], [last_edge]])
            ...
            bin_counts = disc_labels.value_counts(sort=False)

            # 修复空 bin（如有）
            if (bin_counts == 0).any():
                empty_idx = bin_counts[bin_counts == 0].index[0]
                if empty_idx == 0:
                    merge_start, merge_end = 0, 2
                elif empty_idx == self.n_bins - 1:
                    merge_start, merge_end = self.n_bins - 2, self.n_bins
                else:
                    merge_start, merge_end = empty_idx - 1, empty_idx + 2

                merge_left = time_bins[merge_start]
                merge_right = time_bins[merge_end]

                merge_mask = self.patients_df[self.label_col].between(merge_left, merge_right, inclusive='left')
                merge_values = self.patients_df.loc[merge_mask, self.label_col]

                if len(merge_values) < 3:
                    sub_bins = np.linspace(merge_left, merge_right, 4)
                else:
                    _, sub_bins = pd.qcut(merge_values, q=3, retbins=True, duplicates='drop')
                    sub_bins[0] = merge_left
                    sub_bins[-1] = merge_right

                time_bins = np.concatenate([
                    time_bins[:merge_start],
                    sub_bins,
                    time_bins[merge_end+1:]
                ])
                time_bins = np.unique(time_bins)
                if len(time_bins) != self.n_bins + 1:
                    time_bins = np.linspace(full_min - eps, full_max + eps, self.n_bins + 1)
            ...
            # # 等距方式先生成 n_bins-1 个区间边界（产生 n_bins 个点）
            # fixed_bins = np.linspace(
            #     self.label_data[self.label_col].min() - eps,
            #     max_time,
            #     num=self.n_bins
            # )

            # # Step 2: 用分位数方式决定最后一个 bin 的右边界
            # # 得到全体时间的 max 分位 bin 右界（通常比 max_time 大）
            # _, q_bins = pd.qcut(
            #     uncensored_df[self.label_col],
            #     q=self.n_bins,
            #     retbins=True,
            ...
            raise ValueError(f"Unsupported self.discretize_strategy '{self.discretize_strategy}'. Use 'quantile' or 'interval'.")
...
class SurvivalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #@TODO what is the difference between tmil_abmil and transmil_wsi
        elif self.modality in ["mlp_per_path_wsi", "abmil_wsi", "abmil_wsi_pathways", "s4mil_wsi","s4mil_wsi_pathways",
        ...
        elif self.modality in ["coattn", "coattn_motcat","coattn_motcat_bezier"]:
            ...
            return (patch_features, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c, clinical_data, mask,case_id)
            ...
            # for i in range(self.num_pathways):
            #     omic_list.append(torch.tensor(self.omics_data_dict["rna"].iloc[idx][self.omic_names[i]]))

            return (patch_features, omic_list, label, event_time, c, clinical_data, mask)
        elif self.modality in ["ProSurv","DisSurv","BezierSurv"]:
            df_small = self.omics_data_dict["rna"][self.omics_data_dict["rna"]["temp_index"] == case_id]
            df_small = df_small.drop(columns="temp_index")
            df_small = df_small.reindex(sorted(df_small.columns), axis=1)
            omics_tensor = torch.squeeze(torch.Tensor(df_small.values))
            ...
            return (patch_features, omics_tensor, label, event_time, c, clinical_data, mask,case_id)
...
class SurvivalDatasetFactory_0731:
    ...
    def _get_split_from_df(self, args, all_splits, split_key: str='train', fold = None, scaler=None, valid_cols=None):
        ...
        if split_key == "train":
            ...
            sample=False
        split_dataset = SurvivalDataset_0731(
            split_key=split_key,
            fold=fold,
            study_name=args.study,
            modality=args.modality,
...
class SurvivalDataset_0731(Dataset):
    ...
    def __init__(self,
        ...
        super(SurvivalDataset_0731, self).__init__()
        ...
        self.preloaded_patch_features = {}   # {case_id: (patch_tensor, mask)}
        self.preloaded_omics_features = {}   # {case_id: omics_tensor}

        print(f"[INFO] Preloading features to {self.device}...")

        if self.modality in ["BezierSurv","survpath","ProSurv"]:
            for idx in range(len(self.metadata)):
                case_id = self.metadata['case_id'][idx]
                slide_ids = self.patient_dict[case_id]

                # === Load WSI features from all slides for this patient
                patch_features, mask = self._load_wsi_embs_from_path(self.data_dir, slide_ids)
                self.preloaded_patch_features[case_id] = (patch_features.to(self.device), mask.to(self.device))
                ...
                df_small = df_small.reindex(sorted(df_small.columns), axis=1)
                omics_tensor = torch.squeeze(torch.Tensor(df_small.values))
                self.preloaded_omics_features[case_id] = omics_tensor.to(self.device)
    ...
    def __getitem__(self, idx):
        ...
        #@TODO what is the difference between tmil_abmil and transmil_wsi
        elif self.modality in ["mlp_per_path_wsi", "abmil_wsi", "abmil_wsi_pathways", "deepmisl_wsi", "s4mil_wsi","s4mil_wsi_pathways",
                               "deepmisl_wsi_pathways","dsmil_wsi","dsmil_wsi_pathways", "mlp_wsi", "transmil_wsi", "transmil_wsi_pathways"]:
            ...
            # patch_features, mask = self._load_wsi_embs_from_path(self.data_dir, slide_ids)
            patch_features, mask = self.preloaded_patch_features[case_id]
            omic_list = []
            for i in range(self.num_pathways):
                omic_list.append(torch.tensor(self.omics_data_dict["rna"][self.omic_names[i]].iloc[idx]))
            ...
            patch_features, mask = self.preloaded_patch_features[case_id]
            omics_tensor = self.preloaded_omics_features[case_id]
            return (patch_features, omics_tensor, label, event_time, c, clinical_data, mask, case_id)
    ...
    def compute_stats(self):
        ...
        Returns:
            {
              'dataset': str,                 # 使用 study_name
              'non_censored_ratio': float,   # 非删失占比 (0~1)
              'patients': int,               # 病人数量
              'wsis': int,                   # WSI 数
              'patches': int,                # patch 总数
              'patches_per_wsi': float       # 平均每张 WSI 的 patch 数
            }
        约定: self.metadata[self.censorship_var] == 1 表示 censored, 0 表示 uncensored。
             若编码相反，把 non_cens_ratio 的计算改为 cens.mean()。
        """
        # 患者数
        patients = len(self.metadata)

        # WSI 数
        case_ids = list(self.metadata['case_id'])
        wsis = int(sum(len(self.patient_dict[cid]) for cid in case_ids))

        # patch 总数（逐 slide 读取 .pt 仅取行数；不改变现有缓存）
        # 如需更快且允许近似，也可用 self.preloaded_patch_features[cid][0].shape[0] 累加。
        patches = 0
        for cid in case_ids:
            slide_ids = self.patient_dict[cid]
            for sid in slide_ids:
                wsi_path = os.path.join(self.data_dir, f"{sid.rstrip('.svs')}.pt")
                try:
                    bag = torch.load(wsi_path, map_location='cpu')
                    patches += int(bag.shape[0])
                except Exception as e:
                    # 若缺文件或读取失败，跳过并提示
                    # print(f"[WARN] cannot read {wsi_path}: {e}")
                    pass

        # 非删失占比
        cens = self.metadata[self.censorship_var].astype(float).values
        non_cens_ratio = 1.0 - cens.mean()   # 若 1=uncensored, 0=censored，则改成：non_cens_ratio = cens.mean()

        patches_per_wsi = (patches / wsis) if wsis > 0 else 0.0

        return dict(
            dataset=str(self.study_name),
            non_censored_ratio=non_cens_ratio,
            patients=patients,
            wsis=wsis,
            patches=patches,
    ...
    # === NEW: 生成 LaTeX 表格行 ===
    def to_latex_row(self, dataset_name=None):
        """
        输出示例：
        TCGA-BRCA & 13.5\% & 978 & 1,043 & 2,887,441 & 2,768.4 \\\\
        """
        s = self.compute_stats()
        name = dataset_name if dataset_name is not None else s['dataset']
        return (f"{name} & "
                f"{s['non_censored_ratio']*100:.1f}\\% & "
                f"{s['patients']} & "
                f"{s['wsis']} & "
                f"{s['patches']:,} & "
                f"{s['patches_per_wsi']:.1f} \\\\")
